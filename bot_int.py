...
class BotCore:
    def __init__(self, config: Any):  # КОММЕНТАРИЙ: Здесь будет использоваться Config
        self.bot = Bot(token=config.BOT_TOKEN)
        self.dp = Dispatcher(storage=MemoryStorage())
        self.router = Router()  # Создаем роутер для aiogram 3.x
    ...
    def _setup_handlers(self) -> None:
        # Новая система регистрации обработчиков для aiogram 3.x
    ...
    async def _start_handler(self, message: types.Message) -> None:
        ...
        try:
            ...
            if len(message.text.split()) > 1:  # Замена message.get_args()
